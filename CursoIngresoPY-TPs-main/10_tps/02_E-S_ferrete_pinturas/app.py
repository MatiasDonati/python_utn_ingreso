...
class App(customtkinter.CTk):
    ...
    def __init__(self):
        ...
        self.btn_convertir_f_c = customtkinter.CTkButton(master=self, text="Limpiar", command=self.btn_limpiar_on_click)
        self.btn_convertir_f_c.grid(row=4, pady=10, columnspan=2, sticky="nsew")


    def btn_limpiar_on_click(self):
        self.txt_temperatura_f.delete(0, 10000)
        self.txt_temperatura_c.delete(0, 10000)

    def btn_convertir_c_f_on_click(self):
        grados_centigrados_texto = self.txt_temperatura_c.get()
        grados_centigrados_numero = int(grados_centigrados_texto)
        conversion = int((grados_centigrados_numero * 9/5) + 32)
        self.txt_temperatura_f.insert(0, conversion)


    def btn_convertir_f_c_on_click(self):
        fahrenheit_texto = self.txt_temperatura_f.get()
        fahrenheit_numero = int(fahrenheit_texto)
        conversion = int((fahrenheit_numero - 32) * 5/9)
        self.txt_temperatura_c.insert(0, conversion)
