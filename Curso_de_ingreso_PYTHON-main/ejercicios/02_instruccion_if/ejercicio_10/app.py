...
class App(customtkinter.CTk):
    ...
    def btn_mostrar_on_click(self):
        ...
        numero_en_string = str(numero_aleatorio)
        if(numero_aleatorio >= 6 and numero_aleatorio <= 10):
            alert(message="Promoción directa, la nota es " + numero_en_string)
        elif(numero_aleatorio == 4 or numero_aleatorio == 5):
            alert(message="Aprobado, la nota es " + numero_en_string)
        else:
            alert(message="Desaprobado, la nota es " + numero_en_string)
